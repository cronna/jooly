...
<div class="basket-page">
                    ...
                    <?php if(Yii::$app->user->id === $user_basket_product->user_id){ 
                        ...
                        <script>
                            let fullPrice = document.querySelector(".full-price"),
                                ...
                                prodPrice = document.querySelector('.b-c-price')

                            let number = [<?= $fullPrice ?>];
                            const pricesStr = number.toLocaleString(
                                'ru-RU', {style: 'currency', currency: "RUB"}
                            )
                            let number2 = [<?= $product->price ?>];
                            const pricesStr2 = number2.toLocaleString(
                                'ru-RU', {style: 'currency', currency: "RUB"}
                            )
                            

                            fullPrice.textContent = pricesStr
                            totalPrice.textContent = pricesStr
                            prodPrice.textContent = pricesStr2
                        </script>
                    <?php }?>
                <?php }?>
            <?php }?>
    ...
    <div class="basket-info">
        <h3 class="b-i-title">Cумма заказа</h3>
        <div class="fullPrice info-item">
            <span> <b><?= $i ?> товара</b></span>
            <span class='full-price'></span>
        </div>
        <div class="discount info-item">
            <span><b>Cкидка</b></span>
            <span>0 %</span>
        </div>
        <div class="totalPrice info-item">
            <span><b>Итого</b></span>
            <span class="total-price"></span>
        </div>

        <?php 
            $_SESSION['user_products'] = $user_products;
            $_SESSION['fullPrice'] = $fullPrice;
        ?>
        <a href="/orders/add?user_id=<?= Yii::$app->user->id ?>" class="btn">Перейти к оформлению</a>
    </div>
</div>
